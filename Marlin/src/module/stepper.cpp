...
/**
 *Stepper.cpp- 스테퍼 모터를 사용하여 모션 계획을 실행하는 싱글 톤 개체
* 말린 펌웨어
*
* GRBL에서 파생
* Copyright (C) 2009-2011 Simen Svale Skogsrud
*
* grbl은 무료 소프트웨어입니다. 재분배 및/또는 수정할 수 있습니다.
* GNU 일반 공개 라이센스의 조건에 따라
* 라이센스의 버전 3 버전 3, 또는
* (선택에 따라) 이후 버전.
*
* GRBL은 유용하기를 희망하여 배포됩니다.
* 그러나 보증없이;묵시적 보증조차없이
* 특정 목적을위한 상업성 또는 체력.참조
* 자세한 내용은 GNU 일반 공개 라이센스.
*
* GNU 일반 공개 라이센스 사본을 받았어야합니다.
* grbl과 함께.그렇지 않은 경우 <https://www.gnu.org/licenses/>를 참조하십시오.
*/

/**
 * Zack Smith의 'Reprap Cartesian Firmware'가 알리는 타이머 계산
* 및 Philipp Tiefenbacher.
 */
 ...
 *                           time ----->
 *
 *  사다리꼴은 시간이 지남에 따라 속도 곡선의 모양입니다.block-> initial_rate에서 시작하여 가속합니다
* 첫 번째 블록-> accelerate_until step_events_completed.
* step_events_completed는 사다리꼴 생성기가 재설정 될 때까지 감소 된 후에 블록-> decelerate_after에 도달합니다.
* 가속 기울기는 v = u +를 사용하여 계산됩니다. 여기서 t는 지금까지 단계의 누적 타이머 값입니다.
*/

/**
* 말린은 Bresenham 알고리즘을 사용합니다.이론에 대한 자세한 설명을 위해
* 메소드 https://www.cs.helsinki.fi/group/goa/mallinnus/lines/bresenh.html을 참조하십시오
*/

/**
* Eduardo José Tagle의 2018 년 4 월을 추가 한 Bek Controlled Movements Planner는 2018 년 4 월을 추가했습니다.
* Synthethos Tinyg2 소스를 기반으로 한 방정식이지만 고정점
* 가변 기간으로 ISR을 실행할 때 구현은 새로운 것입니다.
* ARM 어셈블러에서 Bézier Velocity Curve 평가를 구현했습니다.
* ISR 속도에 영향을 미치지 않습니다.
 */
